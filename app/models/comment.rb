...
class Comment < ActiveRecord::Base
  ...
  # see above for info on the rescue returns
    ...
    if blacklist_items and blacklist_items.length > 0
      ...
      for i in blacklist_items
        # check for this hit
        if (self.user_ip and self.user_ip.index(/#{i.item}/))
        # found it!
          spam = true
          logger.warn("[Blacklist #{Time.sl_local.strftime('%m-%d-%Y %H%:%M:%S')}]: IP #{self.user_ip} is blacklisted, blocking comment.")
	...
	def before_create
	  ...
	  self.approved = (Preference.get_setting('COMMENTS_APPROVED') == 'yes' ? true : false)
	  if (Preference.get_setting('akismet_filter') == 'yes') then
  ...
  def self.find_for_feed
    self.find(:all, :conditions => 'approved = true', :order => 'created_at desc', :limit => 20)
  end
  ...
  def self.find_unapproved
    self.find(:all, :conditions => ['approved = ?', false])
  end
