...
class PostController < ApplicationController
  ...
  def add_comment
    ...
    # grab the user's requeset paramaters
	  @comment.request = request
    # save it (gets checked for spam as well)
		if @comment.save
		# comment was saved successfully
		  if (Preference.get_setting('COMMENTS_APPROVED') != 'yes') and (Preference.get_setting('AKISMET_FILTERING') != 'yes')
		  # approval is required, notify accordingly
		    ...
		    if Preference.get_setting('AKISMET_FILTERING') != 'no'
  		    if @comment.approved?
  		      flash[:notice] = "Thanks for the comment."
  		    else
  		      flash[:error] = "Unfortnately this comment is considered spam." + 
